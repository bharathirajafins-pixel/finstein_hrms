...
from finstein_hrms import api, scheduled_tasks
...
class TestFoodQR(FrappeTestCase):
	...
	def test_scan_food_qr_marks_consumed_and_increments_qr_count(self):
		...
		with patch("finstein_hrms.api.now_datetime") as mocked_now:
			...
			mocked_now.return_value = datetime.combine(
				getdate(self.today), api.DEFAULT_TIME_WINDOWS["Breakfast"]["start"]
			)
			result = api.scan_food_qr(qr_doc.qr_data)

		updated = frappe.get_doc(
			"Food Count",
			frappe.db.get_value("Food Count", {"user": self.test_user, "order_date": self.today}, "name"),
		)
