...
def validate_interview_scheduling(doc, method):
	...
	round_interview_type = frappe.db.get_value("Interview Round", doc.interview_round, "interview_type")
	if round_interview_type and round_interview_type != doc.custom_interview_type:
		frappe.throw(
			_(
				"Interview Round '{0}' does not belong to Interview Type '{1}'. "
				"Please select a matching Interview Round."
			).format(doc.interview_round, doc.custom_interview_type)
		)
	...
	if scheduled_date < current_date:
		frappe.throw(_("Scheduled Date cannot be a past date. Please select today or a future date."))
	...
	if to_time_str <= from_time_str:
		frappe.throw(_("To Time must be greater than From Time. Please select a valid time range."))
	...
	if scheduled_date == current_date:
		...
		if from_time_str < now_str:
			frappe.throw(
				_("From Time cannot be a past time for today. Please select a current or future From Time.")
			)

		if to_time_str < now_str:
			frappe.throw(
				_("To Time cannot be a past time for today. Please select a current or future To Time.")
			)
	...
	existing_interview = frappe.db.get_value("Interview", filters, ["name", "status"], as_dict=True)

	if existing_interview:
		frappe.throw(
			_(
				"An interview for round '{0}' already exists for this job applicant.\n"
				"Existing Interview: {1}\n"
				"Current Status: {2}\n"
				"Only one interview per round is allowed."
			).format(doc.interview_round, existing_interview.name, existing_interview.status or "Pending")
		)

	# -----------------------------------------------
	# 4️⃣ & 5️⃣ Validate Round Order + Previous Round Statuses
	# -----------------------------------------------
	round_order = frappe.db.get_value("Interview Round", doc.interview_round, "round_order")

	if not round_order:
		frappe.throw(
			_(
				"Round order is not defined for Interview Round: '{0}'. "
				"Please configure the round order before scheduling."
			).format(doc.interview_round)
		)
	...
	for prev_round_num in range(1, round_order):
		...
		if not prev_round_name:
			frappe.throw(
				_(
					"Interview Round with order {0} under Interview Type '{1}' is not found. "
					"Please ensure all rounds are configured sequentially."
				).format(prev_round_num, doc.custom_interview_type)
			)

		# Get previous interview for this applicant
		prev_interview = frappe.db.get_value(
			"Interview",
			{"job_applicant": doc.job_applicant, "interview_round": prev_round_name, "docstatus": ["!=", 2]},
			["name", "status"],
			as_dict=True,
		)
		...
		if not prev_interview:
			frappe.throw(
				_(
					"Round {0} ({1}) has not been scheduled yet for this applicant.\n"
					"Please schedule and complete all previous rounds before proceeding."
				).format(prev_round_num, prev_round_name)
			)
		...
		if prev_interview.status == "Pending":
			frappe.throw(
				_(
					"Cannot schedule Round {0}.\n"
					"Previous Round {1} ({2}) is still Pending.\n"
					"Please complete the previous round first."
				).format(round_order, prev_round_num, prev_round_name)
			)
		...
		if prev_interview.status == "Rejected":
			frappe.throw(
				_(
					...
					"Interview process cannot continue for this applicant."
				).format(round_order, prev_round_num, prev_round_name)
			)
		...
		if prev_interview.status != "Cleared":
			frappe.throw(
				_(
					...
					"All previous rounds must be 'Cleared' before scheduling the next round."
				).format(round_order, prev_round_num, prev_round_name, prev_interview.status or "Pending")
			)
