import frappe
from frappe.utils import date_diff, flt, get_time, getdate, nowtime, today
...
def validate_leave_dates(doc, method=None):
	...
	if not allow_past and (from_date < current_date or to_date < current_date):
		frappe.throw("Past date leave is not allowed. Please select today or a future date.")

	if not doc.half_day and from_date == current_date and nowtime() >= str(cutoff):
		frappe.throw(
			f"Full-day leave for today can only be applied before {cutoff}. Please apply for a future date."
		)
	...
	if doc.half_day:
		...
		if half_day_date == current_date and nowtime() >= str(half_cut):
			frappe.throw(f"Half-day leave for today can only be applied before {half_cut}.")
	...
	if doc.custom_from_time and doc.custom_to_time:
		if get_time(doc.custom_from_time) >= get_time(doc.custom_to_time):
			frappe.throw(
				"Leave from time must be earlier than to time. Please check the time fields and try again."
			)
...
def check_leave_balance(doc, settings):
	...
	if not allocation:
		frappe.throw(f"No active leave allocation found for {doc.leave_type}. Please contact HR.")
