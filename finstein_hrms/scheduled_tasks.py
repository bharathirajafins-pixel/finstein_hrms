...
def get_meal_windows():
	...
	breakfast_open = (
		str(settings.meal_breakfast_open) if settings and settings.meal_breakfast_open else "09:00:00"
	)
	breakfast_close = (
		str(settings.meal_breakfast_close) if settings and settings.meal_breakfast_close else "11:00:00"
	)
...
def generate_food_qr_for_date(target_date, food_type=None, force_regenerate=False):
	...
	for meal_type, food_item in meal_items.items():
		...
		existing_name = frappe.db.get_value("Food QR", {"date": target_date, "food_type": meal_type}, "name")
...
def update_food_qr_count(doc, method=None):
	...
	for food_type, (selected_field, status_field) in field_map.items():
		...
		qr_name = frappe.db.get_value("Food QR", {"date": order_date, "food_type": food_type}, "name")
		if qr_name:
...
def escalate_pending_approvals():
	...
	for leave in pending_leaves:
		...
		frappe.logger().info(f"[Escalation] Reminder sent to {recipient} for leave {leave.name}")
...
def lock_attendance_for_processed_payroll():
	...
	frappe.db.commit()
	frappe.logger().info(f"[PayrollLock] Locked attendance for {len(submitted_payrolls)} payroll period(s).")
