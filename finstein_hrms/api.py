...
def scan_food_qr(qr_data):
	...
	if not (window["start"] <= now_time <= window["end"]):
		frappe.throw(f"Outside serving window. {qr_food_type} is only available {_window_str(qr_food_type)}.")
	...
	if not food_count_name:
		frappe.throw("No order found for you today. You must place your order before collection.")
	...
	if not is_selected:
		frappe.throw(f"You did not order {qr_food_type} for today. Only ordered meals can be collected.")
...
def withdraw_leave_application(docname):
	...
	if doc.workflow_state not in ["Pending", "Pending HR Approve"]:
		frappe.throw(
			f"This application cannot be withdrawn at the '{doc.workflow_state}' stage. Please contact HR."
		)
...
def cancel_meal_order(qr_name):
	...
	if not settings.enable_meal_cancellation:
		frappe.throw("Meal order cancellation is not enabled. Please contact HR.")
	...
	if current_status not in ("", "Pending"):
		frappe.throw(f"This order cannot be cancelled. Current status: {current_status}.")
