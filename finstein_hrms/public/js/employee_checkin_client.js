...
function _render_buttons(frm) {
	...
	if (!_can_use_self_checkin()) {
		frm.dashboard.add_comment(
			__("Self check-in is available only for employee users."),
			"orange"
		);
		frm.set_read_only();
		return;
...
function _show_one_status(frm) {
	...
	if (!doc.time) {
		...
		frm.dashboard.add_indicator(
			__("On permission break since {0}. Click Permission Check-In to resume work.", [
				frappe.datetime.str_to_user(doc.break_start),
			]),
			"orange"
		);
	} else {
		...
		frm.dashboard.add_indicator(
			__("Checked In ✅  {0}  |  {1} hrs elapsed  |  {2} hrs to checkout", [
				frappe.datetime.str_to_user(doc.time),
				elapsed,
				remaining,
			]),
			"green"
		...
		frappe.show_alert(
			{
				message: __("{0}  |  {1} hrs  |  {2}", [
					isForce ? "⚡ Force Checkout" : "🔴 Checked Out",
					netHrs,
					status,
				]),
				indicator: color,
...
function _do_break_start(frm) {
	frappe.confirm(
		__("Permission Checkout for <b>{0}</b>? Break time will NOT count as working hours.", [
		...
		() => {
			...
			frm.save("Save").then(() => {
				frappe.show_alert(
					{
						message: __("🟡 Break started at {0}", [frappe.datetime.str_to_user(now)]),
						indicator: "orange",
					},
					5
				);
	...
	frappe.confirm(
		...
		() => {
			...
			frm.save("Save").then(() => {
				frappe.show_alert(
					{
						message: __("✅ Work resumed. Total break: {0} hrs", [breakHrs]),
						indicator: "green",
					},
					5
				);
