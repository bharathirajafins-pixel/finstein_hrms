				...
				frappe.db.get_value(
					...
					function (r) {
						if (
							r &&
							r.interview_type &&
							r.interview_type !== frm.doc.custom_interview_type
						) {
							frm.set_value("interview_round", null);
							frappe.show_alert(
								{
									message: __(
										"Interview Round was cleared because it does not belong to the selected Interview Type."
									),
									indicator: "orange",
								},
								5
			...
			frappe.db.get_value(
				...
				function (r) {
					if (r && r.interview_type) {
						...
						if (
							!frm.doc.custom_interview_type ||
							frm.doc.custom_interview_type !== r.interview_type
						) {
